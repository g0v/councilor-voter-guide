...
zip["高雄市"] = (_zip$ = { '新興區': 800, '前金區': 801, '苓雅區': 802, '鹽埕區': 803, '鼓山區': 804, '旗津區': 805, '前鎮區': 806, '三民區': 807, '楠梓區': 811, '小港區': 812, '左營區': 813, '仁武區': 814, '大社區': 815, '岡山區': 820, '路竹區': 821, '阿蓮區': 822, '田寮區': 823, '燕巢區': 824, '橋頭區': 825, '梓官區': 826, '彌陀區': 827, '永安區': 828, '湖內區': 829, '鳳山區': 830, '大寮區': 831, '林園區': 832, '鳥松區': 833, '大樹區': 840, '旗山區': 842, '美濃區': 843, '六龜區': 844, '內門區': 845, '杉林區': 846, '甲仙區': 847, '桃源區': 848 }, _defineProperty(_zip$, '三民區', 849), _defineProperty(_zip$, '茂林區', 851), _defineProperty(_zip$, '茄萣區', 852), _zip$);
...
function InitAddress() {
    ...
    zone2.onchange = function () {
        if (this.selectedIndex > 0) document.getElementById('zipcode').value = zip[zone1.value][zone2.value];
    };

    // if(getCookie('addr2'))
    // $('#zone2').val(getCookie('addr2'))

    this.set_currentOptions = function () {

        if (getCookie('addr1')) $('#zone1').val(getCookie('addr1'));
        zone1.onchange();
        setTimeout(function () {
            if (getCookie('addr2')) $('#zone2').val(getCookie('addr2'));
        }, 200);
    };
}

//第一個選項
function first(lab ='縣/市' ) {
    var o = document.createElement('option');
    o.text = lab;
    o.value = "";
    return o;
}
function first2(lab ='鄉/鎮/市區') {
    var o = document.createElement('option');
    o.text = lab;
    o.value = "";
    return o;
...
function InitStores(data) {
    ...
    select1.options.add(first());
    select2.options.add(first2('櫃點'));

    var d = _.groupBy(data , 'city');
    for(let i in d){
       //console.log(decodeURI(i) + '....' + d[i])    
        for(let k in d[i]){
           // console.log(decodeURI( d[i][k].name) )    
        }
    }
    //市
    for (let i in d) {
    ...
    let order = {'台北市':1, "新北市":2 , "桃園市":3 ,"新竹市":4, "台中市":5 , "嘉義市":6 , "台南市":7 
                , "高雄市":8 , "屏東縣":9 ,"宜蘭縣":10 , "花蓮縣":11};
    for(var i in order){
    ...
    select1.onchange = function () {
        ...
        if (this.selectedIndex > 0) {
            ...
            for (let i in d[this.value]) {
               // console.log(decodeURI (d[this.value][i].name))
                var o = document.createElement('option');

                o.text = decodeURI (d[this.value][i].name);
                o.value = d[this.value][i].storeid;
