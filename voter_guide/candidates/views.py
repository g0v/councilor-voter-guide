...
def counties(request, election_year):
    counties = Candidates.objects.filter(election_year=election_year).values('county')\
                                 .annotate(constituency=Count('constituency', distinct = True), candidates=Count('id'))\
                                 .order_by('-candidates')
