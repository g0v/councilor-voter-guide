...
def district(request, election_year, county, constituency):
    ...
    candidates = Terms.objects.filter(election_year=election_year, county=county, type='councilors', constituency=constituency).select_related('candidate', 'elected_councilor').order_by('-councilor_terms' if election_year == coming_ele_year else '-votes')
    standpoints = {}
