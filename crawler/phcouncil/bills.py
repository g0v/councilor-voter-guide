...
class Spider(scrapy.Spider):
    ...
    def parse_profile(self, response):
        ...
        item['id'] = '%s-%s' % (self.election_year, re.search(u'id=([^&]*)', response.url).group(1))
        for key, label in [('category', u'類[\s　]*別'), ('abstract', u'案[\s　]*由'), ('description', u'說[\s　]*明'), ('methods', u'辦[\s　]*法'), ('execution', u'決[\s　]*議'), ('execution', u'議[\s　]*決')]:
        ...
        if item['type'] == u'縣府提案':
            ...
            item['proposed_by'] = re.split(u'[，、 ]', re.sub(u'(副?議長|議員)', '', response.xpath(u'//*[re:test(., "(提[\s　]*案|動[\s　]*議|請[\s　]*願)[\s　]*人")]')[-1].xpath('following-sibling::td[1]/text()').extract_first()).strip())
        item['petitioned_by'] = re.split(u'[，、 ]', re.sub(u'(副?議長|議員)', '', response.xpath(u'//td[re:test(., "(連[\s　]*署|附[\s　]*議)[\s　]*人")]')[-1].xpath('following-sibling::td[1]/text()').extract_first()).strip()) if len(response.xpath(u'//*[re:test(., "(連[\s　]*署|附[\s　]*議)[\s　]*人")]')) > 0 else ['']
        item['links'] = [
            {
                'url': response.url,
                'note': 'original'
