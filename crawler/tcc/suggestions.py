...
class Spider(scrapy.Spider):
    ...
    def parse(self, response):
        yield response.follow(response.xpath(u'//a[@title="政府資訊公開"]/@href').extract_first(), callback=self.parse_opengov)

    def parse_opengov(self, response):
        yield response.follow(response.xpath(u'//a[contains(text(), "議員所提地方建設建議事項")]/@href').extract_first(), callback=self.parse_file)

    def parse_file(self, response):
        for node in response.xpath(u'//a[contains(@title, "年度")]'):
            item = {}
