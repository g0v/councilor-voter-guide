...
class Spider(scrapy.Spider):
    ...
    start_urls = ["http://tycc.digital.tpa.gov.tw/"]
    download_delay = 0.5
    county_abbr = os.path.dirname(os.path.realpath(__file__)).split('/')[-1]
    election_year = common.election_year(county_abbr)
    ads = {'2009': u'桃園縣.*?第(十七|17)屆', '2014': u'桃園市.*?第(一|1)屆', '2018': u'桃園市.*?第(二|2)屆'}
    ...
    def parse_login(self, response):
        return response.follow(response.xpath(u'//a[re:test(., "^提案$")]/@href').extract_first(), callback=self.parse_query, headers=common.headers(self.county_abbr))
