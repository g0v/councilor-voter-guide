...
class Spider(scrapy.Spider):
    ...
    def parse_list(self, response):
        for url in response.xpath(u'//a[re:test(text(), "12月底議員建議事項")]/@href').extract():
    ...
    def parse_file(self, response):
        ...
        m = re.search(u'(?P<year>\d+)年', title)
        ...
        for node in response.css('.div_file').xpath('a'):
            ...
            cmd = u'mkdir -p ../../data/hsinchucc/suggestions/ && wget --heade="User-Agent:Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36" -q -O ../../data/hsinchucc/suggestions/{year}_{month_from}-{month_to}.{file_ext} "{url}"'.format(**item)
