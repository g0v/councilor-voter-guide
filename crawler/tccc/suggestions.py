    ...
    def parse_list(self, response):
        for node in response.xpath(u'//a[re:test(@title, "\d+[上下]半年")]'):
            ...
            if re.search(u'檔案下載', node.xpath('@title').extract_first()):
                ...
                cmd = u'mkdir -p ../../data/tccc/suggestions/ && wget --heade="User-Agent:Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36" -Nc -O ../../data/tccc/suggestions/{year}_{month_from}-{month_to}.{file_ext} "{url}"'.format(**item)
