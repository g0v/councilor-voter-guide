...
class Spider(scrapy.Spider):
    name = 'suggestions'
    start_urls = ["http://www.kinmen.gov.tw/KinmenWeb/wSite/page/1.html"]

    def parse(self, response):
        yield scrapy.Request(urljoin(response.url, response.xpath(u'//a[contains(text(), "議員所提")]/@href').extract_first()), callback=self.parse_link)

    def parse_link(self, response):
        yield scrapy.Request(urljoin(response.url, response.css('.List_Link a').xpath('@href').extract_first()), callback=self.parse_list)
    ...
    def parse_download(self, response):
        ...
        item['file_ext'] = response.headers['Content-Disposition'].split('.')[-1]
